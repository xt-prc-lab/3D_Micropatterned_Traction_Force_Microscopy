%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                                                                              %
%       Function used to cross-correlate a timeseries of 3D stack images (beads) to a reference 3D stack image (trypsin)       %
%           and correct their relative shift so it is lower than 0.5 pixels. The 3D stack images are then cropped to           %
...
function [Settings, File] = cell_cropper_3D_full(Settings, File)
    ...
    for ii=1:stepbeads:File.NFiles.Beads
        ...
        if ~exist([File.CroppPath, filesep, 'crop', '_', 'Beads', '_', num2str(ii), '.tif'], 'file') || re_calculate || ...
            ...
            % Find the relative displacement of the reference and deformed stacks.
            ...
            [~, ~, ~, shx_temp, shy_temp, shz_temp, MaxCorr] = disp_on_blocks_fast_3D(...
                ...
                num_planes, ...                                 % Resolution of the PIV grid, in z.
                0, ...                                          % Overlap between blocks, in y.
                0, ...                                          % Overlap between blocks, in x.
                0, ...                                          % Overlap between blocks, in z.
                padding, ...                                    % Padding for the PIV, in y, usually 0.
                padding, ...                                    % Padding for the PIV, in x, usually 0.
            ...
            if (File.Drift(ii).pkh > Settings.MinCorrelationTrheshold) && ...
                ...
                % Crop the "file_type" image.
                file_type = {'Fluo', 'Fluo_1', 'Fluo_2'};

                for jj = 1:numel(file_type)


                    if  Settings.Do.Open.(file_type{jj})
                        
                        padval = 0;
                        
                        % Load the TIFFStack object.
                        tsStack = TIFFStack(File.Name(ii).(file_type{jj}));
                        
                        File.Name(ii).(['Crop', file_type{jj}]) = [File.CroppPath, filesep, 'crop', '_', file_type{jj}, '_', ...
                                                                   num2str(ii), '.tif'];
                        
                        % Array to store the padded Fluo_1 image. Since it is larger than the original Fluo_1 image, we will 
                        % store only the padded array. This way, we will save  a lot of RAM for large stacks.
                        ...
                        if (floor(abs(shz_temp)) >= paddingz) && (ind0 > 1) && (indf < num_images2)
                            ...
                            im3((1:Settings.SizeY)+padding, (1:Settings.SizeX)+padding, ind0_d:indf_d) = ...
                                im3_corr(:, :, ind0:indf);
                        ...
                        % Crop "file_type" image.
                        % Shift the matrix im3 and store it again in the same variable. This will save a considerable ammount 
                        % of RAM for larger stacks.
                        ...
                        imwrite(uint16(im3((i1:i2)+padding, (j1:j2)+padding, 1+paddingz)), ...
                                File.Name(ii).(['Crop', file_type{jj}]), 'compression', 'none');
                        
                        for kk=2:num_planes
                            imwrite(uint16(im3((i1:i2)+padding, (j1:j2)+padding, kk+paddingz)), ...
                                    File.Name(ii).(['Crop', file_type{jj}]), 'WriteMode', 'append', 'compression', 'none');
                        end
